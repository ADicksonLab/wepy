...
class OpenMMSimMaker:
    ...
    def make_state(self, system, positions):
        ...
        init_state = gen_walker_state(
            positions, system, integrator, getState_kwargs=self.getState_kwargs
        )
    ...
    def make_apparatus(
        ...
        runner = OpenMMRunner(
            self.system, self.topology, integrator, platform=platform, **runner_params
        )

        # RESAMPLER

        # choose which resampler to use
        resampler_class = [res for res in self.RESAMPLERS if res.__name__ == resampler][
            0
        ]
        ...
        resampler = resampler_class(
            distance=self.distance, init_state=self.init_state, **resampler_params
        )
        ...
        # build the apparatus
        sim_apparatus = WepySimApparatus(
            runner, resampler=resampler, boundary_conditions=bc
        )
    ...
    def choose_dashboard_sections(self, apparatus):
        ...
        if apparatus.boundary_conditions is not None:
            dashboard_sections = {
                "resampler": ResamplerDashboardSection(apparatus.resampler),
        ...
        if type(apparatus.resampler).__name__ == "WExploreResampler":
            dashboard_sections["resampler"] = WExploreDashboardSection(
                apparatus.resampler
            )
        # REVO
    ...
    def resolve_reporter_params(self, apparatus, reporter_specs, reporters_kwargs=None):
        ...
        for reporter_spec in reporter_specs:
            ...
            if reporter_spec == "WepyHDF5Reporter":
                ...
                reporter_params.update(
                    deepcopy(self.DEFAULT_REPORTER_PARAMS[reporter_spec])
                )
    ...
    def make_configuration(
        ...
        # use the spec string given
            ...
            work_mapper_class = [
                mapper for mapper in self.MAPPERS if mapper.__name__ == work_mapper_spec
            ][0]
        ...
        reporter_classes, reporter_params = self.resolve_reporter_params(
            apparatus, reporters, reporter_kwargs
        )
...
class OpenMMToolsTestSysSimMaker(OpenMMSimMaker):
    ...
    def box_vectors(cls):
        ...
        return (
            np.array([bv.value_in_unit(unit.nanometer) for bv in bvs]) * unit.nanometer
        )
