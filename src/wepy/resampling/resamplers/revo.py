...
class REVOProjectionResampler(REVOResampler):
    ...
    is used to guide merging, and a sum of "single-body" terms is used
    as the variation function V.  The single-body terms are also calculated 
    using the walker projections.

    This algorithm was originally proposed here:
    https://doi.org/10.1063/5.0239713

    The objective function in that work was defined as
    .. math::
        V = \sum_{i} V_i = \sum_i \frac{1}{d^i_{\text{from_target}}}

    and here it is implemented such that any quantity can be used
    ...
    ensemble of walkers and performs the follow steps:
       ...
       - Creates the resampling data that includes
           ...
           - walker_variations : the Vi values for each walker

           - walker_projections : the projections for each walker

           - variation : the final value of trajectory variation (V)
    ...
    def __init__(
        ...
        **kwargs
        ...
        max_function : function (float, np.array) -> float
            Uses the weight and a projection for one walker (np.array) as input, returns Vi.
            The resampler will promote maximization of this quantity.
        ...
        # superclass. We set the min and max number of walkers to be
        # constant
        super(REVOResampler,self).__init__(
            pmin=pmin,
            pmax=pmax,
            min_num_walkers=Ellipsis,
            max_num_walkers=Ellipsis,
    ...
    def _calc_variation(self, walker_weights, num_walker_copies, projection_list):
        """Calculates the variation value.
        ...
        num_walker_copies : list of int
            ...
            1 means there is one copy of this walker.
            >1 means it will be cloned to this number of walkers.
        ...
        for i in range(num_walkers):
            if num_walker_copies[i] > 0:
                variation += num_walker_copies[i]*self.max_function(walker_weights[i]/num_walker_copies[i],projection_list[i])
                walker_variations[i] = self.max_function(walker_weights[i]/num_walker_copies[i],projection_list[i])

        return variation, walker_variations

    def decide(self, walker_weights, num_walker_copies, distance_matrix, projection_list):
        """Optimize the trajectory variation by making decisions for resampling.
        ...
        # calculate the initial variation which will be optimized
        variation, walker_variations = self._calc_variation(
            walker_weights, new_num_walker_copies, projection_list
        )
        variations.append(variation)
        ...
        return walker_actions, variations[-1], walker_variations

    def _all_to_all_distance(self, projection_list):
        """Calculate the pairwise all-to-all distances between walkers.

        Parameters
        ----------
        projection_list : list of numpy arrays


        Returns
        -------
        distance_matrix : np.array(n_walkers,n_walkers)

        """
        # initialize an all-to-all matrix, with 0.0 for self distances
        n_walkers = len(projection_list)
        dist_mat = np.zeros((n_walkers, n_walkers))

        for i in range(n_walkers-1):
            for j in range(i,n_walkers):
                dist_mat[i,j] = np.sqrt(np.sum(np.square(projection_list[i] - projection_list[j])))
                dist_mat[j,i] = dist_mat[i,j]
        
        return dist_mat

    def _calc_projection_list(self, walkers):
        if self.num_proc > 1:
            pool = mulproc.Pool(self.num_proc)
            projs = pool.map(self.projector.project, [walker.state for walker in walkers])
        else:
            projs = []
            for walker in walkers:
                projs.append(self.projector.project(walker.state))
        
        return projs

    def resample(self, walkers):
        """Resamples walkers based on REVOProjectionResampler algorithm
        ...
        # calculate projection list
        projection_list = self._calc_projection_list(walkers)

        # calculate distance matrix
        distance_matrix = self._all_to_all_distance(projection_list)

        # determine cloning and merging actions to be performed, by
        # maximizing the variation, i.e. the Decider
        resampling_data, variation, walker_variations = self.decide(
            walker_weights, num_walker_copies, distance_matrix, projection_list
        )
