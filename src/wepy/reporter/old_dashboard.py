...
"""
    ...
    def __init__(self,
        ...
        assert runner is not None, "must provide the runner"
        assert resampler is not None, "must provide the resampler"

        assert issubclass(type(runner), OpenMMRunner), "Only supports the OpenMM runner"

        self.runner = runner
        self.resampler = resampler
        self.bcs = boundary_conditions

        # get the values we need from them
        self.step_time = runner.
    ...
    def _weighted_ensemble_string(self):
        weighted_ensemble_string = self.WEIGHTED_ENSEMBLE_TEMPLATE.format(
            init_date_time=self.init_date_time.isoformat(),
    ...
    def _boundary_conditions_string(self):
        ...
        warping_log = self.BOUNDARY_CONDITIONS_TEMPLATE.format(
            cutoff_distance=(self.bc_cutoff_distance
                             if self.bc_cutoff_distance is not None
                             else "not given"),
