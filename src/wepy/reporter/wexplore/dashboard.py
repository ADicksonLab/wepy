...
"""
    ...
    def update_values(self, **kwargs):
        ...
        for resampler_record in kwargs["resampler_data"]:
            ...
            branch_record = (new_leaf_id, branching_level, walker_image_distance)
    ...
    def gen_fields(self, **kwargs):
        ...
        region_hierarchy = "\n".join(["{}     {}" for i in range(len(regions))]).format(
            *region_children_pairs
        )
        ...
        region_table_d["n_walkers"] = [
            self.curr_region_counts[region] for region in self.region_ids
        ]
        region_table_d["curr_weight"] = [
            self.curr_region_probabilities[region] for region in self.region_ids
        ]
        leaf_region_table_df = pd.DataFrame(
            region_table_d, columns=region_table_colnames
        )
        leaf_region_table_df.set_index("region", drop=True)

        leaf_region_table_str = tabulate(
            leaf_region_table_df,
            headers=leaf_region_table_df.columns,
            tablefmt="orgtbl",
        )

        # log of branching events
        branching_table_colnames = (
            "new_leaf_id",
            "branching_level",
            "trigger_distance",
        )
        branching_table_df = pd.DataFrame(
            self.branch_records, columns=branching_table_colnames
        )
        branching_table_str = tabulate(
            branching_table_df, headers=branching_table_df.columns, tablefmt="orgtbl"
        )
        ...
        walker_table_str = tabulate(
            walker_table_df, headers=walker_table_df, tablefmt="orgtbl"
        )
