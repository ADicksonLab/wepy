...
def test_H5_warping():
    ...
    if len(warp_pos['positions']) > 0:
        ...
        warp_next_trace = [(wr.walker_idx,wr.cycle_idx + 1) for wr in wr_list if wr.cycle_idx + 1 < n_cycles]
        with we:
...
def test_H5_resampling():
    ...
    with we:
        ...
        all_pos = np.array([we.h5[f'runs/0/trajectories/{i}/positions'] for i in range(n_walkers)])
        all_wts = np.array([we.h5[f'runs/0/trajectories/{i}/weights'] for i in range(n_walkers)])
    ...
    for cr in clone_rrs:
        ...
        if cycle_idx + 1 < n_cycles:
            for target in targets:
                ...
                assert all_wts[parent][cycle_idx]/len(targets) == all_wts[target][cycle_idx+1]

                # test if the children are within segment_length of the parent
                assert np.sum(np.abs(all_pos[parent][cycle_idx] - all_pos[target][cycle_idx+1])) <= segment_length
    ...
    for km in keep_merge_rrs:
        ...
        if cycle + 1 < n_cycles:
            ...
            squashed_walkers = [sr.walker_idx for sr in squash_rrs if (sr.target_idxs[0] == walker and sr.cycle_idx == cycle)] + [walker]

            # check if the sum of their weights (on this cycle) equals the km weight (on the next cycle)
            np.testing.assert_almost_equal(np.sum(all_wts[walker][cycle+1]), np.sum(all_wts[squashed_walkers,cycle]), decimal=5)
...
def test_H5_contig():
    ...
    with we:
        ...
        n_walkers = we.num_walkers(0,0)
        all_pos = np.array([we.h5[f'runs/0/trajectories/{i}/positions'] for i in range(n_walkers)])
        ct = ContigTree(we,boundary_condition_class=RandomWalkBC, decision_class=MultiCloneMergeDecision)  
    ...
    for trace in sw:
        ...
        assert np.sum(np.abs(all_pos[trace[1][1],trace[1][2]] - all_pos[trace[0][1],trace[0][2]])) <= 1
        assert np.sum(np.abs(all_pos[trace[2][1],trace[2][2]] - all_pos[trace[1][1],trace[1][2]])) <= 1
    ...
    for lin in lineages:
        for i in range(len(lin)-1):
            # test if positions are adjacent
            assert np.sum(np.abs(all_pos[lin[i][1],lin[i][2]] - all_pos[lin[i+1][1],lin[i+1][2]])) <= 1
    ...
    for lin in disc_lineages:
        for i in range(len(lin)-1):
            ...
            if np.sum(np.abs(all_pos[lin[i][1],lin[i][2]] - all_pos[lin[i+1][1],lin[i+1][2]])) > 1:
