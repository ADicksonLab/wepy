...
@pytest.mark.usefixtures('lj_reporters',
                         'lj_init_walkers',
                         'lj_openmm_runner',
                         'lj_unbinding_bc',
...
class TestLJSimIntegration():
    ...
    # context.
    @pytest.mark.parametrize('boundary_condition_class', ['UnbindingBC',])
    @pytest.mark.parametrize('resampler_class', ['WExploreResampler', ])
    @pytest.mark.parametrize('platform', ['Reference', 'CPU', 'OpenCL',]) # 'CUDA'
    @pytest.mark.parametrize('work_mapper_class', ['WorkerMapper', 'TaskMapper', 'Mapper',])
    def test_lj_sim_manager_openmm_integration_run(self, class_tmp_path_factory,
        ...
        reporters = Configuration(work_dir=tmpdir,
        ...
        boundary_condition = [boundary_condition for boundary_condition in boundary_condition_fixtures
                   if type(boundary_condition).__name__ == boundary_condition_class][0]
        resampler = [resampler for resampler in resampler_fixtures
                   if type(resampler).__name__ == resampler_class][0]
        ...
        work_mapper_classes = {mapper_class.__name__ : mapper_class for
                               mapper_class in [Mapper, WorkerMapper, TaskMapper]}
        ...
        if work_mapper_class == 'Mapper':
            ...
            if platform == 'CUDA':
                work_mapper = TaskMapper(num_workers=num_workers,
        ...
        walkers, filters = manager.run_simulation(n_cycles, steps,
