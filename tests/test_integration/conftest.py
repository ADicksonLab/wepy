...
from wepy.runners.openmm import OpenMMRunner, OpenMMState, OpenMMWalker
...
# old one
from wepy.orchestration.orchestrator import Orchestrator as OldOrchestrator
...
def pytest_configure(config):
    config.addinivalue_line(
        "markers", "interactive: tests which give you the debugger"
    )



#import wepy_examples.lennard_jones as lj
...
# # Lennard Jones (lj) system

# @pytest.fixture(scope='class')
# def lj_params():

#     return copy(lj.params)
...
@pytest.fixture(scope='class')
def lj_integrator(lj_params):
    integrator = omm.LangevinIntegrator(lj_params['temperature'],
                                        lj_params['friction_coefficient'],
                                        lj_params['step_size'],
    )
...
@pytest.fixture(scope='class')
def lj_init_sim_state():
    return copy(lj.init_sim_state)

@pytest.fixture(scope='class')
def lj_init_state(lj_init_sim_state):

    init_state = OpenMMState(lj_init_sim_state)

    return init_state


@pytest.fixture(scope='class')
def lj_openmm_runner(lj_params, lj_omm_sys, lj_integrator):

    # initialize the runner
    runner = OpenMMRunner(lj_omm_sys.system,
                          lj_omm_sys.topology,
                          lj_integrator,
                          platform=lj_params['platform'])
