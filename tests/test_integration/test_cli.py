...
class TestCLI():
    ...
    def test_run_orch(self, function_tmp_path_factory, lj_orchestrator_defaults_file):
        ...
        result = runner.invoke(wepy_cli, ['run', 'orch', '--job-dir', workdir,
                                          orch_path, start_hash, n_seconds, n_steps],
                               catch_exceptions=False,)

        assert result.exit_code == 0

    def test_run_snapshot(self, function_tmp_path_factory, lj_snapshot, lj_configuration):
        ...
        result = runner.invoke(wepy_cli, ['run', 'snapshot', '--job-dir', workdir,
                                          snap_path,config_path, n_seconds, n_steps],
                               catch_exceptions=False,)

        assert result.exit_code == 0

    def test_get_snapshot(self, function_tmp_path_factory, lj_orchestrator_defaults_file):
        ...
        result = runner.invoke(wepy_cli, ['get', 'snapshot',
                                          orch_path, start_hash],
                               catch_exceptions=False,)
    ...
    def test_get_config(self, function_tmp_path_factory, lj_orchestrator_defaults_file):
        ...
        result = runner.invoke(wepy_cli, ['get', 'config',
                                          orch_path, config_hash],
                               catch_exceptions=False,)
    ...
    def test_get_run(self, function_tmp_path_factory, lj_orch_file_orchestrated_run):
        ...
        result = runner.invoke(wepy_cli, ['get', 'run',
                                          orch_path, start_hash, end_hash],
                               catch_exceptions=False,)
    ...
    def test_reconcile(self, function_tmp_path_factory,
                       lj_orch_file_orchestrated_run,
                       lj_orch_file_other_orchestrated_run):
        ...
        result = runner.invoke(wepy_cli, ['reconcile', 'orch', '--hdf5', h5_target,
                                          orch_target, orch_path, other_orch_path],
                               catch_exceptions=False)
