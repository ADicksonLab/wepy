...
As from before the test drive script is installed as a module and can be
...
pieces, parts, and objects you will need to interact with.

Running a Simulation and Producing Results
------------------------------------------

The test drive is configured so that all of the file outputs are dropped
...
You may also see some Warnings that can safely be ignored until you want
...
The ``root.walkers.dcd`` can be loaded on top of the
``root.init_top.pdb`` file to show you a snapshot of what the last
ensemble of walkers looked like.
...
For instance we can run again using the REVO ``--resampler`` by using
the resampler flag:
...
You can even disable using a resampler entirely by using the dummy "No"
resampler. This is actually quite useful for comparing improvements in
sampling using resampling:
...
like ``h5py``, but can also be other tools like ``h5ls`` or ``hdfql``
which are agnostic to the particular schemas the data has.

However, ``wepy`` provides an API to ``WepyHDF5`` files (I like to use
the file extension ``*.wepy.h5``) that makes accessing WE specific data
much easier.
...
When constructing this class you can use it to make new HDF5 files from
scratch, however this should be done for you in the reporter that
generates HDF5 files.

So for us we only use it to read HDF5 files. Run this code to construct
...
automatically close it and protect the data.
...
access slices or chunks of them as you need.
...
this when exporting for visualization etc.
...
The main category of data are the "runs" that are stored in the HDF5.
Each "run" dataset corresponds to a single self-contained simulation.
...
for now we will just focus on them one run at a time.

To list the IDs (just indices really) we can look at the ``run_idxs``
attribute:

   Throughout ``wepy`` 'idx' is used as shorthand for 'index' or 'idxs'
   for 'indices'.
...
Now that we know which runs we are interested in we can query more data
about it.
...
Lets pick one of the trajectories (0) from the run (0) and see what data
it has available.
...
formats that can be used for visualization.

Note that We still haven't loaded any more data than the metadata for a
...
get it just for this one trajectory:
...
Here we have asked for the box vectors of trajectory 0 in run 0. This is
returned as a ``numpy`` array of rank 3. The shape of the array
...
Here this is a list of exactly the frame indices we want:
...
different runners can all use the same format.
...
``mdtraj.Trajectory`` objects. These require the presence of the
...
You can even use the ``_output/run0/root.init_top.pdb`` file and the
``_output/run0/traj0.dcd`` files to view them in something like VMD.
...
But there is also a module full of utilities that can help with working
with these topology files. For example we can view them as ``pandas``
dataframe tables based on the atoms, residues, or chains. For this
example, only the atoms are interesting.
...
requirements would be too great to store entire simulations.
...
field) most of the convenience functions in the ``WepyHDF5`` interface
...
killed then you might see a conspicuous change from the last location.
...
couple other analysis routines and classes in ``wepy``. So we will defer
this question for now. However, we will cover the building blocks that
are used in conjunction with those higher-level tools.

First, we will introduce the idea of a **trace** which is a way to
specify a set of frames across runs, trajectories, and cycles.

For example using the same dataset we might be interested in the all of
the walker states for a particular cycle, e.g. the last one:
...
us. Such is the case with the ``last_cycle_trace`` above, it could just
as easily be a ``set``. Typically unless otherwise specified the order
of the trace will always be preserved.

Somewhat confusingly there are different kinds of traces depending which
collection you are indexing. Here each tuple value corresponds to
``(run_idx, traj_idx, cycle_idx)`` and thus can be used to make traces
...
this mistake and not realize it.
...
length of the trace rather than the "trajectory".

Just to give an example of the different kinds of traces we can get the
same data but relative to only this run:
...
When dealing with continuous trajectories of walkers using traces is the
...
When we don't need to worry about continuous trajectories it makes sense
to just think of the runs and trajectory datasets.
...
This method returns a generator so that only one trajectory dataset will
...
For example we might have a function that gets the distance between the
two Lennard-Jones particles in the simulation:
...
While we won't cover it here you can optionally save this data back into
the HDF5 file so that it aligns to the walker-frame that it was computed
on. This is useful for some of the more advanced analyses.
...
These can easily be retrieved for a single run by running the following
method:
...
The ``[0]`` we passed in is just the single ``run_idx`` for our run.
This methods can be used to stitch together more than one run, but that
isn't of interest at the moment.

It should look something like this depending on what happened in your
simulation:
...
As an end user you shouldn't ever need to interact with this raw data as
...
Of the columns all are necessary except the ``region_assignment`` one.
That column is specific to the ``WExplore`` resampler and there would be
different extra columns when using a different resampler like ``REVO``.
