...
*** COMMENT Installing Tooling

# TOREV: I think this isn't necessary since we have these things in
# the virtual env. Or we could include an arbitrary script into the
# envs folder for each env if we need to.

To get bootstrapped into the toolchain run the ~tooling.bash~ file:
