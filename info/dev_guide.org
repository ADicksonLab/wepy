...
#+BEGIN_SRC bash
git clone https://github.com/salotz/python-fstab
cd python-fstab
#+END_SRC
...
To create an env called ~python-fstab.dev~ just run the ~env~ target from
~invoke~:
...
#+begin_src bash
pip install --index-url https://test.pypi.org/simple/ --no-deps python-fstab-salotz
#+end_src
