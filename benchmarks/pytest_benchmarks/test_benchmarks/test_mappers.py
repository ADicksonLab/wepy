...
# 5 ps, 10 ps, 20 ps
N_STEPS_TEST = [100] # [5000, 10000, 20000]
N_CYCLES_TEST = [100]
SYSTEMS_TEST = [ 'LysozymeImplicit',] # 'LennardJonesPair',
PLATFORMS_TEST = ['CPU'] #['OpenCL'] #['Reference', 'CPU', 'OpenCL']
...
class TestBenchmark():
    ...
    def test_mappers(self, n_walkers, n_cycles, n_workers, n_steps, platform, system, mapper,
                    benchmark):
        ...
        config = sim_maker.make_configuration(work_mapper=mapper,
                                              platform=platform,
                                              apparatus=apparatus)
        ...
        def thunk():
            ...
            return sim_manager.run_simulation(n_cycles, n_steps,
...
class TestSimpleBenchmark():
    ...
    @pytest.mark.parametrize('n_walkers', N_WALKER_TESTS)
    def test_Mapper(self, n_walkers,
                          benchmark):
    ...
    @pytest.mark.parametrize('n_walkers', N_WALKER_TESTS)
    @pytest.mark.parametrize('n_workers', N_WORKER_TESTS)
    def test_WorkerMapper(self, n_walkers, n_workers,
                                 benchmark):

        mapper = WorkerMapper(segment_func=task_pass,
                              num_workers=n_workers,
                              worker_type=Worker)
    ...
    @pytest.mark.parametrize('n_walkers', N_WALKER_TESTS)
    @pytest.mark.parametrize('n_workers', N_WORKER_TESTS)
    def test_TaskMapper(self, n_walkers, n_workers,
                                 benchmark):

        mapper = TaskMapper(segment_func=task_pass,
                            num_workers=n_workers,
