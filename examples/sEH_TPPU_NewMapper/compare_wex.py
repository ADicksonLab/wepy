...
if __name__ == "__main__":
    ...
    d1 = distance_matrix
    pmax = 0.2
    resampler = WExplore2Resampler(reference_traj=pdb,seed=5000,pmax=pmax)
    ...
    print ("second----------------------------------------------")
    resampler = newsampler(reference_traj=pdb,seed=5000,pmax=pmax )
    walkerwt = [ 1/n_walkers for i in range(n_walkers)]
    amp = [ 1 for i in range(n_walkers)]
    resampler.decide_clone_merge(n_walkers,walkerwt,amp,d1,debug_prints=True)
